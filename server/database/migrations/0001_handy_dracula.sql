CREATE TABLE `tags` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`tag` text NOT NULL,
	`enabled` integer DEFAULT true NOT NULL
);
--> statement-breakpoint
PRAGMA foreign_keys=OFF;--> statement-breakpoint
CREATE TABLE `__new_images` (
	`key` text PRIMARY KEY NOT NULL,
	`tags` text NOT NULL,
	`nsfw` integer DEFAULT false NOT NULL,
	`favorite` integer DEFAULT false NOT NULL,
	`alive` integer DEFAULT true NOT NULL,
	`createDate` text NOT NULL,
	`deleteDate` text
);
--> statement-breakpoint
INSERT INTO `__new_images`("key", "tags", "nsfw", "favorite", "alive", "createDate", "deleteDate") SELECT "key", "tags", "nsfw", "favorite", "alive", "createDate", "deleteDate" FROM `images`;--> statement-breakpoint
DROP TABLE `images`;--> statement-breakpoint
ALTER TABLE `__new_images` RENAME TO `images`;--> statement-breakpoint
PRAGMA foreign_keys=ON;